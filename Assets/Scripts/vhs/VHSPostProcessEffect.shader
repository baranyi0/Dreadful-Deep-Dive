		...
		SubShader{
			Pass {
				...
				float rand(float3 co) {
					 return frac(sin(dot(co.xyz ,float3(0,0,0))));
				}
				...
				fixed4 frag(v2f_img i) : COLOR{
					...
					dy = ((int)(dy * 15)) / 15.0;
					dy = dy;
					i.uv.x += dy * 0.00 + rand(float3(dy,dy,dy));

					//float white = (vhs.r + vhs.g + vhs.b) / 1;
					...
					float bleed = tex2D(_MainTex, i.uv + float2(0.00, 0)).r;
